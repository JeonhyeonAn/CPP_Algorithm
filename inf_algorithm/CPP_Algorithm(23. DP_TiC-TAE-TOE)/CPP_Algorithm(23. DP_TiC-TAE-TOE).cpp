// CPP_Algorithm(23. DP_TiC-TAE-TOE).cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
#include <iostream>
#include <vector>
#include <list>
...
int CanWin(vector<vector<char>>& board, char turn)
{
	...
	// 캐시 확인
	int key = HashKey(board);   // 강의 다시 확인
	int& ret = cache[key];
	if (ret != DEFAULT)
		return ret;
	...
	for (int y = 0; y < 3; y++)
	{
		for (int x = 0; x < 3; x++)
		{
			...
			// 착수
			board[y][x] = turn;   // 계속 복사하면 낭비->&로 원본으로 테스트 -> 나중에 다시 취소
...
int main()
{
	...
	// memset은 1byte 단위로 밀어준다.  0, -1 은 4byte나 1byte나 같기 때문에 사용 가능
